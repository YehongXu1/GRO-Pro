...
```text
scripts/build_mh_real_100k_congestion_candidates.sh
scripts/run_mh_real_100k_congestion_diagnostic.sh
scripts/run_mh_real_100k_windows_gro.sh
scripts/run_mh_real_100k_windows_paper_baselines.sh
...
data/MH_Real_query_sets_100k_window6h
data/MH_Real_query_sets_100k_window3h
data/MH_Real_query_sets_100k_window2h
data/MH_Real_query_sets_100k_window1h
data/MH_Real_query_sets_100k_window30min
```
...
For BJ real-world overall effectiveness, the previously used three 100k
representative datasets are now diagnostic only:
...
Do not use these as the final paper-facing real-world overall-effectiveness
workloads unless the user explicitly chooses to keep them. They have suitable
congestion ratios, but their free-flow shortest-path average is only about
`3.55` minutes per query, so the OD lengths are too short for a convincing
real-world route-optimization workload.

The revised final BJ real-world plan is:

```text
query_count fixed at 100k
ODs derived from real T-Drive trajectory segments
network free-flow shortest-path avg >= 8-10 minutes
network free-flow shortest-path avg <= 45-60 minutes as an upper guard
traffic intensity chosen by evaluated_ttt / free_flow_ttt
lower / middle / high generated by departure-window variants, not by Rep count
```

Candidate output directories should be named clearly, for example:
...
See `notes/bj_real_queries.md` for the revised gates. Do not use
`BJRealRep5-*` for overall effectiveness, because those files have only 50k
queries. Do not use `Rep1/Rep5/Rep10` as traffic-intensity levels for final
real-world tables.

The old BJ diagnostic overall-effectiveness scripts and CSVs have been removed
to avoid confusing them with the final paper-facing workload. Recreate BJ
overall-effectiveness runners only after the longer-trip 100k candidates have
been selected.
...
After the diagnostic, choose three 100k files by `avg_free_flow_tt` and
`inflation_ratio`; only then launch MH paper baselines and proposed GRO on
those selected files. The older `window6h_all` MH scripts/results were removed
because they mixed 10k, 50k, and 100k query counts.
...
4. Overall effectiveness against paper baselines.
   ...
   - Final real-world setting should use fixed-100k query sets with realistic
     OD lengths. For BJ, regenerate longer-trip real candidates before using
     the overall-effectiveness table; the old lower/middle/extreme BJ files are
     diagnostic only because their free-flow average is about 3.55 minutes.
   - For MH, use fixed-100k `MHRealRep10-*` candidates and choose traffic
     levels by both free-flow query length and congestion ratio.
...
- Real workload status: the old BJ real lower/middle/extreme files are now
  diagnostic only. They have fixed 100k query count and useful congestion
  ratios, but the free-flow shortest-path average is only about 3.55 minutes,
  so the query length is too short for the final paper-facing real-world table.
  Regenerate BJ real candidates with a network free-flow query-length gate
  before rerunning overall effectiveness. Keep the old labels attached to the
  exact files only when discussing previous diagnostic runs.
...
Current overall-effectiveness entrypoints are for MH fixed-100k window
candidates. They run one window after another and support `LOG=...`:
...
Run proposed GRO and iterative no-TDG baselines on all current MH 100k windows:
...
Run one-shot paper baselines on all current MH 100k windows:
...
The old BJ `three_levels` and MH `window6h_all` scripts/results were removed
because they correspond to diagnostic workloads, not the final
